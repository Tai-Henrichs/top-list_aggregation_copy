...
master += [f'python3 sim.py [FootRule+,Borda+,Score-Then-Borda+,RandomSort,Local-Search,Relaxed-Linear-Program,Copeland,Optimal] s [{n},{N},{th},{k*n}] {SEED}'
            for k in ks_ratio
            for n in ns
            for N in Ns
            for th in ths
...
master += [f'python3 sim.py [Score-Then-Adjust,Score-Then-Adjust-Relaxed,{ep}] s [{ns[LOW]},{Ns[LOW]},{ths[HIGH-1]},{ks_ratio[LOW]*ns[LOW]:.1f}] {SEED}'
            for ep in epsilons
            ]

#n=45, N=2000, th=0.001, k=n*0.7
master += [f'python3 sim.py [Score-Then-Adjust,Score-Then-Adjust-Relaxed,{ep}] s [{ns[HIGH]},{Ns[HIGH]},{ths[LOW-1]},{ks_ratio[HIGH]*ns[HIGH]:.1f}] {SEED}'
            for ep in epsilons
            ]
